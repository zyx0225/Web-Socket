...
#define MYPORT "32992"	// the port client1 will use to connect directory server
#define SERVERPORT 31992	// the port client1 will be connecting to in phase 1
#define MAXBUFLEN 100 

#define DS_HN "nunki.usc.edu"
#define FS_HN "nunki.usc.edu"

char C1_IP[100],DS_IP[100],FS_IP[100];
char C1_HN[128];
...
static int phase2(void)
{
	...
	int rv;
	int numbytes;
	char msg[100]="Client1 doc1"; /*msg send to directory server*/

	gethostname(C1_HN, sizeof C1_HN);
	hostname_to_ip(DS_HN, DS_IP);    //convert hostname to ip address
	hostname_to_ip(C1_HN, C1_IP); 
	...
	if ((rv= getaddrinfo(C1_HN, MYPORT, &hints, &clientinfo)) != 0) {
	...
	printf("Phase 2: The File requested by Client 1 is present in %s and the File Server's TCP port number is %s.\n",fs,port);
...
static int phase3(void)  //phase 3
{
	...
	if ((rv = getaddrinfo(C1_HN, NULL, &hints, &clientinfo)) != 0) {   //use dynamic port number
		fprintf(stderr, "getaddrinfo: %s\n", gai_strerror(rv));
		return 1;
	}
	...
	if (send(sockfdc, "Client1 doc1", 12, 0) == -1)
		perror("Phase 3: send to file server");

	printf("Phase 3: The file request from Client 1 has been sent to the %s.\n", fs);
	...
	printf("Phase 3: Client 1 received %s from %s.\n", buf, fs);

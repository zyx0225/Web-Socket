...
#define MYPORT_UDP "23992"	// the port file server2 will use to connect directory server
#define SERVERPORT 21992	// the port file server2 will be connecting to
#define MYPORT_TCP "42992"  //the port file server2 will use in phase 3
...
static int phase1()
{
	...
	int rv;
	int numbytes;
	char msg[100]="File_Server2 42992"; /*msg send to directory server*/
	
	gethostname(FS2_HN, sizeof FS2_HN);
	hostname_to_ip(DS_HN, DS_IP);    //convert hostname to ip address
	hostname_to_ip(FS2_HN, FS2_IP); 
	...
	if ((rv = getaddrinfo(FS2_HN, MYPORT_UDP, &hints, &clientinfo)) != 0) {
	...
	struct sockaddr_in fs2addr;
	socklen_t fs2addr_len = sizeof(fs2addr);

	getsockname(sockfdc,(struct sockaddr *)&fs2addr,&fs2addr_len); //get the port number and IP add

	printf("Phase 1: File Server 2 has UDP port number %d and IP address %s. \n",ntohs(fs2addr.sin_port), inet_ntoa(fs2addr.sin_addr));
...
static int phase3()  //phase 3 for file server
{
	...
	if ((rv = getaddrinfo(FS2_HN, MYPORT_TCP, &hints, &servinfo)) != 0) {
		fprintf(stderr, "getaddrinfo: %s\n", gai_strerror(rv));
		return 1;
	}
	...
	getsockname(sockfd,(struct sockaddr *)&fs2addr,&fs2addr_len); //get the port number and IP add

	printf("Phase 3: File Server 2 has TCP port number %d and IP address %s.\n",ntohs(fs2addr.sin_port), inet_ntoa(fs2addr.sin_addr));
	...
	while(1) {  // main accept() loop
		...
		if (!fork()) { // this is the child process
			...
			printf("Phase 3: File Server 2 received the request from the %s for the file %s.\n", client, file);
			printf("Phase 3: File Server 2 has sent %s to %s.\n", file, client);

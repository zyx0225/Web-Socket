...
#define MYPORT_UDP "22992"	// the port file server1 will use to connect directory server
#define SERVERPORT 21992	// the port file server1 will be connecting to
#define MYPORT_TCP "41992"  //the port file server1 will use in phase 3
...
static int phase1()
{
	...
	int rv;
	int numbytes;
	char msg[100]="File_Server1 41992"; /*msg send to directory server*/
	
	gethostname(FS1_HN, sizeof FS1_HN);
	hostname_to_ip(DS_HN, DS_IP);    //convert hostname to ip address
	hostname_to_ip(FS1_HN, FS1_IP); 
	...
	if ((rv = getaddrinfo(FS1_HN, MYPORT_UDP, &hints, &clientinfo)) != 0) {
	...
	struct sockaddr_in fs1addr;
	socklen_t fs1addr_len = sizeof(fs1addr);

	getsockname(sockfdc,(struct sockaddr *)&fs1addr,&fs1addr_len); //get the port number and IP add

	printf("Phase 1: File Server 1 has UDP port number %d and IP address %s. \n",ntohs(fs1addr.sin_port), inet_ntoa(fs1addr.sin_addr));
...
static int phase3()  //phase 3 for file server
{
	...
	if ((rv = getaddrinfo(FS1_HN, MYPORT_TCP, &hints, &servinfo)) != 0) {
		fprintf(stderr, "getaddrinfo: %s\n", gai_strerror(rv));
		return 1;
	}
	...
	getsockname(sockfd,(struct sockaddr *)&fs1addr,&fs1addr_len); //get the port number and IP add

	printf("Phase 3: File Server 1 has TCP port number %d and IP address %s.\n",ntohs(fs1addr.sin_port), inet_ntoa(fs1addr.sin_addr));
